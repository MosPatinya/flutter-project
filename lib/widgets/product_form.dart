  ...
  Widget build(BuildContext context) {
    return Container(
      child: Column(
        ...
        children: [
          Text(widget.product != null ? 'แก้ไข ${widget.product.productName}' : 'เพิ่มสิ้นค้าใหม่'),
          TextField(
            controller : nameController,
            keyboardType: TextInputType.text,
            decoration: InputDecoration(labelText: 'ชื่อสิ้นค้า'),
          ),
          TextField(
            controller: priceController,
            keyboardType: TextInputType.number,
            decoration: InputDecoration(labelText: 'ราคาสิ้นค้า'),
          ),
          SizedBox(height:10),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
...
Widget _showOkButton(){
  return RaisedButton(
    onPressed: () async {
      String newProductId = 'PD${DateTime.now().millisecondsSinceEpoch.toString()}';
      await db.setProduct(
        product : ProductModel(
        id: widget.product == null ? 
        newProductId: widget.product.id,
